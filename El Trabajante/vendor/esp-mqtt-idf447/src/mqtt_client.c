...
static bool create_client_data(esp_mqtt_client_handle_t client)
{
    ...
    /* AUT-539 Fix 1: TCP keepalive probes — detects half-open connections independent of
     * the 60s MQTT application keepalive. idle=10s, interval=5s, count=3 -> dead-detect
     * in <=25s. Without SO_KEEPALIVE, lwip socket stays in ESTABLISHED after broker closes;
     * PINGREQ hangs in retransmit buffer, no MQTT_EVENT_DISCONNECTED fires -> reconnect-loop
     * breaks after ~20 cycles (lwip FD-pool exhaustion, session_taken_over accumulation). */
    {
        esp_transport_keep_alive_t ka_cfg = {
            .keep_alive_enable = true,
            .keep_alive_idle = 10,
            .keep_alive_interval = 5,
            .keep_alive_count = 3
        };
        esp_transport_tcp_set_keep_alive(tcp, &ka_cfg);
    }
