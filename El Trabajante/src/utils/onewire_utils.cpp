#include "onewire_utils.h"

namespace OneWireUtils {

// ============================================
// CRC8 LOOKUP TABLE (OneWire Standard)
// ============================================
// Polynomial: x^8 + x^5 + x^4 + 1 (0x8C reflected)
// Pre-calculated for performance
// Same table used by OneWire library (OneWire::crc8)
static const uint8_t crc8_table[256] PROGMEM = {
    
};

// ============================================
// INTERNAL HELPER: CRC8 Calculation
// ============================================
static uint8_t calculateCrc8(const uint8_t* data, uint8_t len) {
    uint8_t crc = 0;
    for (uint8_t i = 0; i < len; i++) {
        crc = pgm_read_byte(&crc8_table[crc ^ data[i]]);
    }
    return crc;
}

// ============================================
// ROM TO HEX STRING
// ============================================
String romToHexString(const uint8_t rom[8]) {
    String hex = "";
    hex.reserve(16);  // Pre-allocate for performance
    
    for (uint8_t i = 0; i < 8; i++) {
        if (rom[i] < 0x10) {
            hex += "0";  // Leading zero for single-digit hex
        }
        hex += String(rom[i], HEX);
    }
    
    hex.toUpperCase();
    return hex;
}

// ============================================
// HEX STRING TO ROM
// ============================================
bool hexStringToRom(const String& hex, uint8_t rom[8]) {
    // Validate length (must be exactly 16 hex chars)
    if (hex.length() != 16) {
        return false;
    }
    
    // Parse hex string to bytes
    for (uint8_t i = 0; i < 8; i++) {
        String byteStr = hex.substring(i * 2, i * 2 + 2);
        
        // Check if valid hex characters
        for (uint8_t j = 0; j < 2; j++) {
            char c = byteStr.charAt(j);
            if (!((c >= '0' && c <= '9') || 
                  (c >= 'A' && c <= 'F') || 
                  (c >= 'a' && c <= 'f'))) {
                return false;
            }
        }
        
        // Convert to byte using strtol for safety
        rom[i] = (uint8_t)strtol(byteStr.c_str(), nullptr, 16);
    }
    
    return true;
}

// ============================================
// ROM VALIDATION (CRC8)
// ============================================
bool isValidRom(const uint8_t rom[8]) {
    // Calculate CRC for first 7 bytes
    uint8_t crc = calculateCrc8(rom, 7);
    
    // Compare with CRC in byte 7
    return (crc == rom[7]);
}

// ============================================
// DEVICE TYPE FROM FAMILY CODE
// ============================================
String getDeviceType(const uint8_t rom[8]) {
    switch (rom[0]) {
        case 0x28:
            return "ds18b20";
        case 0x10:
            return "ds18s20";
        case 0x22:
            return "ds1822";
        default:
            return "unknown";
    }
}

}  // namespace OneWireUtils
